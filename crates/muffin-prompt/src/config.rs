//! Configuration for the segmented prompt, parsed from `MUFFIN_PROMPT_*`
...
impl Config {
    ...
    /// shell's `EnvStack`).
    ///
    /// `MUFFIN_PROMPT_LEFT` / `MUFFIN_PROMPT_RIGHT` are comma lists. An explicitly
    /// empty `MUFFIN_PROMPT_LEFT` disables the whole segmented prompt (the driver
    /// falls back to the legacy prompt); unset values use the defaults.
    pub fn from_env(get: &dyn Fn(&str) -> Option<String>) -> Self {
        let left_segments = parse_list(get("MUFFIN_PROMPT_LEFT"), DEFAULT_LEFT);
        let right_segments = parse_list(get("MUFFIN_PROMPT_RIGHT"), DEFAULT_RIGHT);
        ...
        for seg in [
            ...
            let var = alloc::format!("MUFFIN_PROMPT_COLOR_{}", seg.name().to_uppercase());
        ...
        Config {
            ...
            right_segments,
            colors,
            status_fail: get("MUFFIN_PROMPT_COLOR_FAIL")
                .and_then(|s| parse_color(&s))
                .unwrap_or(Color::new(52, 196)),
            git_dirty: get("MUFFIN_PROMPT_COLOR_GIT_DIRTY")
                .and_then(|s| parse_color(&s))
                .unwrap_or(Color::new(236, 229)),
            time_ms: get("MUFFIN_PROMPT_TIME_MS")
                .and_then(|s| s.trim().parse().ok())
                .unwrap_or(TIME_DEFAULT_MS),
            maxlen: get("MUFFIN_PROMPT_MAXLEN")
                .and_then(|s| s.trim().parse().ok())
                .unwrap_or(0),
            dir_alt_fg: get("MUFFIN_PROMPT_DIR_ALT")
                .and_then(|s| s.trim().parse().ok())
                .unwrap_or(39),
            clock_enabled: !matches!(
                get("MUFFIN_PROMPT_CLOCK").as_deref().map(str::trim),
...
mod tests {
    ...
    fn explicit_empty_left_disables() {
        assert!(cfg(&[("MUFFIN_PROMPT_LEFT", "")]).left_segments.is_empty());
        assert!(cfg(&[("MUFFIN_PROMPT_LEFT", "  ")]).left_segments.is_empty());
        assert!(cfg(&[("MUFFIN_PROMPT_LEFT", ",")]).left_segments.is_empty());
    }
    ...
    fn parses_left_and_right_lists() {
        let c = cfg(&[
            ("MUFFIN_PROMPT_LEFT", "dir,git"),
            ("MUFFIN_PROMPT_RIGHT", "status,clock"),
        ]);
    ...
    fn time_maxlen_and_dir_alt() {
        let c = cfg(&[
            ("MUFFIN_PROMPT_TIME_MS", "500"),
            ("MUFFIN_PROMPT_MAXLEN", "30"),
            ("MUFFIN_PROMPT_DIR_ALT", "31"),
        ]);
        assert_eq!(c.time_ms, 500);
        assert_eq!(c.maxlen, 30);
        assert_eq!(c.dir_alt_fg, 31);
        let c = cfg(&[
            ("MUFFIN_PROMPT_TIME_MS", "abc"),
            ("MUFFIN_PROMPT_MAXLEN", "-1"),
            ("MUFFIN_PROMPT_DIR_ALT", "x"),
        ]);
    ...
    fn clock_toggle() {
        ...
        assert!(!cfg(&[("MUFFIN_PROMPT_CLOCK", "0")]).clock_enabled);
        assert!(!cfg(&[("MUFFIN_PROMPT_CLOCK", "off")]).clock_enabled);
        assert!(cfg(&[("MUFFIN_PROMPT_CLOCK", "1")]).clock_enabled);
    }

    #[test]
    fn color_override_and_defaults() {
        let c = cfg(&[("MUFFIN_PROMPT_COLOR_DIR", "236,231")]);
    ...
    fn status_fail_and_git_dirty_colors() {
        ...
        let c = cfg(&[("MUFFIN_PROMPT_COLOR_FAIL", "196,231")]);
        assert_eq!(c.status_fail(), Color::new(196, 231));
        let c = cfg(&[("MUFFIN_PROMPT_COLOR_FAIL", "not-a-color")]);
        assert_eq!(c.status_fail(), Color::new(52, 196));
    }
}
