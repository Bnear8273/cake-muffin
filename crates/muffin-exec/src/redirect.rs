...
use muffin_platform::{ChildFd, Fd, FileOpenMode};
use muffin_syntax::{Redirect, RedirectKind, RedirectTarget};
    ...
    match (&r.kind, &r.target) {
        ...
        (RedirectKind::Write | RedirectKind::Clobber, RedirectTarget::Word(w)) => {
            ...
            let fd = ctx.platform
                ...
                .map_err(|e| alloc::format!("muffin: {path}: {e}"))?;
        ...
        _ => {
            return Err(alloc::format!("muffin: unsupported redirection {:?}", r.kind));
        }
    }
    Ok(())
}
...
fn setup_stdin_string(platform: &dyn muffin_platform::ProcessModel, stdin: &mut ChildFd, text: &str) -> Result<(), String> {
    let (r, w) = platform
    ...
    while written < bytes.len() {
        let n = platform
            ...
            .map_err(|e| alloc::format!("muffin: write: {e}"))?;
        if n == 0 {
            break;
        }
        written += n;
    ...
    platform
        .close(w)
        .map_err(|e| alloc::format!("muffin: close: {e}"))?;
    *stdin = ChildFd::Fd(r);
    Ok(())
}
