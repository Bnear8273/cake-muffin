...
use muffin_blacklist::CommandBlacklist;
use muffin_env::{EnvStack, EnvVar};
use muffin_platform::{ChildFd, Fd, ProcessHandle, SpawnConfig, WaitOptions, WaitStatus};
...
pub(crate) fn word_text(w: &muffin_syntax::Word) -> String {
    use muffin_syntax::WordPart;
...
fn part_text(p: &muffin_syntax::WordPart) -> String {
    match p {
        muffin_syntax::WordPart::DoubleQuoted(parts, _) => {
            let mut s = String::new();
            for inner in parts {
        ...
        _ => word_text(&muffin_syntax::Word {
            parts: alloc::vec![p.clone()],
            span: muffin_syntax::Span::new(0, 0),
        }),
    }
}
...
pub struct Executor<'a> {
    pub platform: &'a dyn muffin_platform::ProcessModel,
    pub env: EnvStack,
    ...
    pub(crate) proc_subst: Vec<muffin_platform::Fd>,
...
impl<'a> Executor<'a> {
    pub fn new(env: EnvStack, platform: &'a dyn muffin_platform::ProcessModel) -> Self {
        Self {
            platform,
            env,
            last_status: ProcStatus::NotStarted,
            positional: alloc::vec!["muffin".into()],
    ...
    /// Evaluate a command string (used by `muffin -c` and the interactive
        ...
        match parse(src) {
            ...
            Err(errs) => {
                let msg = if errs.is_empty() {
                    "muffin: parse error".into()
                } else {
                    alloc::format!("muffin: {}", errs[0].message)
                };
    ...
    /// spawn a fresh `muffin -c` process or use `CreateProcess` + IPC (see
    ...
    fn command_to_text(&self, cmd: &Command) -> String {
        use muffin_syntax::{AssignmentValue, RedirectTarget};
        match &cmd.kind {
            CommandKind::Simple(sc) => {
                ...
                for r in &cmd.redirects {
                    ...
                    redir.push_str(match r.kind {
                        muffin_syntax::RedirectKind::Write => ">",
                        muffin_syntax::RedirectKind::Append => ">>",
                        muffin_syntax::RedirectKind::Read => "<",
                        muffin_syntax::RedirectKind::ReadWrite => "<>",
                        muffin_syntax::RedirectKind::DupInput => "<&",
                        muffin_syntax::RedirectKind::DupOutput => ">&",
                        muffin_syntax::RedirectKind::Heredoc => "<<",
                        muffin_syntax::RedirectKind::HereString => "<<<",
                        muffin_syntax::RedirectKind::Clobber => ">|",
                        muffin_syntax::RedirectKind::AndOut => "&>",
                        muffin_syntax::RedirectKind::AndAppend => "&>>",
                    });
                    match &r.target {
        ...
        for (i, cmd) in pipeline.commands.iter().enumerate() {
            ...
            let next_pipe: Option<(Fd, Fd)> = if is_last {
                ...
                match self.platform.create_pipe_pair(false) {
                    Ok(p) => Some(p),
                    Err(e) => {
                        self.report_error(&alloc::format!("muffin: pipe: {e}"));
                        return ProcStatus::Exit(1);
                    }
                }
            };
        ...
        match &cmd.kind {
            ...
            CommandKind::Subshell(sub) => {
                ...
                match handle {
                    ...
                    Err(e) => Err(alloc::format!("muffin: subshell: {e}")),
                }
            }
    ...
    fn eval_simple(
        ...
        for a in &sc.assignments {
            match &a.value {
                ...
                AssignmentValue::Array(words) => {
                    ...
                    self.env
                        .set(&a.name, muffin_env::EnvVar::new_list(values))
                        .map_err(|e| alloc::format!("muffin: {e}"))?;
                }
                AssignmentValue::Index { index, value } => {
                    let mut ctx = self.ctx();
                    ...
                    set_indexed(self, &a.name, idx, v).map_err(|e| alloc::format!("muffin: {e}"))?;
                }
            }
        }
        ...
        match resolve_command(self, &cmd_name) {
            ...
            CommandSpec::NotFound => {
                ...
                self.report_error(&alloc::format!("muffin: {cmd_name}: command not found"));
    ...
    fn run_external(
        ...
        let handle = self.platform
            .spawn(&cfg)
            .map_err(|e| alloc::format!("muffin: {}: {e}", argv[0]))?;
    ...
    fn eval_if(&mut self, ifc: &muffin_syntax::IfCommand) -> ProcStatus {
    ...
    fn eval_coproc(&mut self, cc: &CoprocCommand) -> ProcStatus {
        ...
        let (c2p_read, c2p_write) = match p.create_pipe_pair(false) {
            Ok(p) => p,
            Err(e) => {
                self.report_error(&alloc::format!("muffin: coproc: pipe: {e}"));
                return ProcStatus::Exit(1);
            }
        };
        ...
        match handle {
            ...
            Err(e) => {
                ...
                let _ = p.close(p2c_write);
                self.report_error(&alloc::format!("muffin: coproc: {e}"));
                ProcStatus::Exit(1)
            }
        }
    }
    ...
    fn apply_fds_in_parent<T>(&mut self, fds: &CommandFds, f: impl FnOnce(&mut Self) -> T) -> T {
        ...
        for (slot, cfg) in [
            ...
            match cfg {
                ...
                ChildFd::File(path) => {
                    if let Ok(f) = p.open_file(path, muffin_platform::FileOpenMode::Write) {
                        if let Ok(s) = p.duplicate_fd(slot) {
                            saved.push((slot, s));
                        }
            ...
            let expand_next = if is_alias_name(word) && !seen.contains(word.as_str()) {
                match self.aliases.get(word).cloned() {
                    Some(val) => {
                        ...
                        match muffin_syntax::split_command_line(&val) {
                            Ok(sub) => {
                                for s in sub {
                                    let w = muffin_syntax::word::parse_word(
                                        &s,
                                        muffin_syntax::Span::new(0, 0),
                                    );
                                    let mut ctx = self.ctx();
...
fn set_indexed(exec: &mut Executor, name: &str, idx: usize, value: String) -> Result<(), String> {
    ...
    values[idx] = value;
    exec.env
        .set(name, muffin_env::EnvVar::new_list(values))
        .map_err(|e| alloc::format!("muffin: {e}"))
}
...
mod tests {
    ...
    fn mock_platform() -> muffin_platform_mock::MockPlatform {
        muffin_platform_mock::MockPlatform::new()
    }
    ...
    fn expands_parameter_in_alias_value() {
        ...
        e.env.set("HOME", muffin_env::EnvVar::new("/root")).unwrap();
