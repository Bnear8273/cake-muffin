...
use muffin_syntax::{Command, Parameter, Word, WordPart};
...
pub struct ExpandCtx<'a> {
    ...
    pub proc_subst_fds: &'a mut Vec<muffin_platform::Fd>,
}
...
fn expand_process_subst(ctx: &mut ExpandCtx, raw: &str) -> Result<PartOut, String> {
    let (is_input, body) = match raw.as_bytes().first() {
        ...
        _ => return Err(alloc::format!("muffin: bad process substitution `{raw}`")),
    };
    let p = ctx.platform;
    let (r, w) = p
        .create_pipe_pair(false)
        .map_err(|e| alloc::format!("muffin: pipe: {e}"))?;
...
fn expand_command_subst(
        ...
        if !path_text.is_empty() {
            ...
            return match crate::builtins::read_file(ctx.platform, &path) {
                ...
                Err(e) => {
                    ...
                    ctx.report_error(&alloc::format!("muffin: {path}: {e}"));
                    Ok(PartOut::Nothing)
                }
            };
        }
    ...
    let _ = p.close(r);
    let _ = p.wait(&handle, muffin_platform::WaitOptions::NONE);
...
fn expand_parameter(
    ...
    if !name.is_empty() && name.chars().all(|c| c.is_ascii_digit()) {
        ...
        return Ok(match params.get(idx.saturating_sub(1)) {
            ...
            None => {
                if ctx.nounset {
                    return Err(alloc::format!("muffin: ${name}: unbound variable"));
                }
                PartOut::Nothing
            }
        });
...
fn expand_indexed(
    ...
    match values.get(n) {
        ...
        None => {
            if ctx.nounset {
                return Err(alloc::format!("muffin: {name}[{idx}]: unbound variable"));
            }
            Ok(PartOut::Nothing)
        }
    }
...
fn apply_param_op(
    ...
    match op {
        ...
        ParamOp::Assign(word, colon) => {
            ...
            if use_default {
                let w = expand_operand(ctx, &word)?;
                let _ = ctx.env.set(name, muffin_env::EnvVar::new(w.clone()));
                Ok(part_value(&Some(w), in_dquotes))
            } else {
                Ok(part_value(&value, in_dquotes))
            }
        ...
        ParamOp::Error(word, colon) => {
            ...
            if unset_or_null {
                ...
                Err(alloc::format!("muffin: {name}: {msg}"))
            } else {
                Ok(part_value(&value, in_dquotes))
            }
        }
        ...
        ParamOp::Indirect => {
            ...
            if inner_name.is_empty() {
                if ctx.nounset {
                    return Err(alloc::format!("muffin: ${name}: invalid indirect expansion"));
                }
                return Ok(PartOut::Nothing);
            }
            let resolved = param_value(ctx, &inner_name);
            if resolved.is_none() && ctx.nounset {
                return Err(alloc::format!("muffin: {inner_name}: unbound variable"));
            }
...
pub fn expand_plain_string(ctx: &mut ExpandCtx, s: &str) -> Result<Vec<String>, String> {
    let word = muffin_syntax::word::parse_word(s, muffin_syntax::Span::new(0, s.len() as u32));
...
pub fn word_from_string(s: &str) -> Word {
    muffin_syntax::word::parse_word(s, muffin_syntax::Span::new(0, s.len() as u32))
}
