...
use muffin_env::{EnvVar, EnvVarFlags};
...
pub fn run_builtin(
    ...
    Some(match name {
        "echo" => echo(exec.platform, args),
        "printf" => printf(exec.platform, args),
        "true" | ":" => Ok(ProcStatus::Exit(0)),
        "false" => Ok(ProcStatus::Exit(1)),
        "exit" => exit(exec, args),
        "cd" => cd(exec, args),
        "pwd" => pwd(exec.platform),
        ...
        "test" => test_builtin(exec.platform, args),
        "[" => test_builtin(exec.platform, args),
...
fn out(platform: &dyn muffin_platform::ProcessModel, s: &str) {
    let _ = platform.write(1, s.as_bytes());
}

// --- echo ---

fn echo(
    platform: &dyn muffin_platform::ProcessModel,
    args: &[String],
...
fn printf(
    ...
    if args.len() < 2 {
        return Err("muffin: printf: usage: printf FORMAT [ARG...]".into());
    }
    let fmt = &args[1];
    let mut buf = String::new();
...
fn exit(exec: &mut Executor, args: &[String]) -> Result<ProcStatus, String> {
    let code = match args.get(1) {
        Some(s) => match s.parse::<i32>() {
            Ok(n) => n & 0xff,
            Err(_) => return Err(alloc::format!("muffin: exit: {s}: numeric argument required")),
        },
...
fn cd(exec: &mut Executor, args: &[String]) -> Result<ProcStatus, String> {
    let old = exec.platform.current_dir();
    let target = match args.get(1) {
        None => match exec.env.get("HOME") {
            Some(v) => v.value().to_owned(),
            None => return Err("muffin: cd: HOME not set".into()),
        },
        Some(d) if d == "-" => match exec.env.get("OLDPWD") {
            Some(v) => {
                out(exec.platform, &alloc::format!("{}\n", v.value()));
                v.value().to_owned()
            }
            None => return Err("muffin: cd: OLDPWD not set".into()),
        },
        Some(d) => d.clone(),
    };
    exec.platform
        .set_current_dir(&target)
        .map_err(|e| alloc::format!("muffin: cd: {target}: {e}"))?;
    exec.env
    ...
    exec.env
        .set(
            "PWD",
            EnvVar::new(exec.platform.current_dir()).set_flags(EnvVarFlags::EXPORT),
        )
        .ok();
    Ok(ProcStatus::Exit(0))
}
...
fn pwd(platform: &dyn muffin_platform::ProcessModel) -> Result<ProcStatus, String> {
    out(platform, &alloc::format!("{}\n", platform.current_dir()));
    Ok(ProcStatus::Exit(0))
}
...
// each session). muffin has no rc file yet, so define them in the interactive
// REPL, e.g.:
//
...
// # Why colour aliases?
//
// muffin's child processes inherit the terminal stdout, so tools that detect a
...
fn alias(exec: &mut Executor, args: &[String]) -> Result<ProcStatus, String> {
    ...
    if i >= args.len() {
        for (name, value) in &exec.aliases {
            print_alias(exec.platform, name, value);
        }
    ...
    for arg in &args[i..] {
        match arg.find('=') {
            ...
            None => match exec.aliases.get(arg) {
                Some(value) => print_alias(exec.platform, arg, value),
                None => return Err(alloc::format!("muffin: alias: {arg}: not found")),
            },
        }
    }
    Ok(ProcStatus::Exit(0))
...
fn unalias(exec: &mut Executor, args: &[String]) -> Result<ProcStatus, String> {
    ...
    for name in &args[1..] {
        if exec.aliases.remove(name).is_none() {
            return Err(alloc::format!("muffin: unalias: {name}: not found"));
        }
    }
    Ok(ProcStatus::Exit(0))
}

fn print_alias(platform: &dyn muffin_platform::ProcessModel, name: &str, value: &str) {
    out(
        platform,
        &alloc::format!("alias {name}='{}'\n", quote_single(value)),
    );
}
...
fn test_builtin(platform: &dyn muffin_platform::ProcessModel, args: &[String]) -> Result<ProcStatus, String> {
    let is_bracket = args.first().map(String::as_str) == Some("[");
    let mut expr = &args[1..];
    if is_bracket {
        if expr.last().map(String::as_str) != Some("]") {
            return Err("muffin: [: missing `]'".into());
        }
    ...
    let val = eval_test_expr(platform, expr)?;
...
fn eval_test_expr(platform: &dyn muffin_platform::ProcessModel, args: &[String]) -> Result<bool, String> {
    let mut p = TestParser { platform, args, pos: 0 };
    let val = p.parse_or()?;
    if p.pos != args.len() {
        return Err(alloc::format!(
            "muffin: test: unexpected argument `{}'",
            args[p.pos]
        ));
    }
    Ok(val)
...
struct TestParser<'a> {
    platform: &'a dyn muffin_platform::ProcessModel,
    args: &'a [String],
    pos: usize,
}
...
impl TestParser<'_> {
    ...
    fn parse_primary(&mut self) -> Result<bool, String> {
            ...
            && is_unary_op(&op)
        {
            ...
            let arg = self
                .eat()
                .ok_or_else(|| "muffin: test: argument expected".to_string())?;
            return unary_test(self.platform, &op, &arg);
        }
        let lhs = self
            .eat()
            .ok_or_else(|| "muffin: test: expression expected".to_string())?;
        match self.peek().map(String::from) {
            Some(bin) if is_binary_op(&bin) => {
                ...
                let rhs = self
                    .eat()
                    .ok_or_else(|| "muffin: test: argument expected".to_string())?;
                binary_test(self.platform, &lhs, &bin, &rhs)
            }
            _ => Ok(!lhs.is_empty()),
        }
    }
...
fn unary_test(platform: &dyn muffin_platform::ProcessModel, op: &str, arg: &str) -> Result<bool, String> {
    Ok(match op {
        "-n" => !arg.is_empty(),
        "-z" => arg.is_empty(),
        "-t" => {
            let fd: u32 = arg.trim().parse().unwrap_or(0);
            platform.is_terminal_fd(fd)
        }
        _ => {
            let info = platform.file_info(arg);
            let my_uid = platform.effective_user_id();
            let my_gid = platform.effective_group_id();
            crate::test_ops::eval_unary_file_test(&info, op, my_uid, my_gid)
        }
    })
}

fn binary_test(platform: &dyn muffin_platform::ProcessModel, lhs: &str, op: &str, rhs: &str) -> Result<bool, String> {
    Ok(match op {
        "=" | "==" => lhs == rhs,
        "!=" => lhs != rhs,
        "-eq" => crate::test_ops::parse_num(lhs) == crate::test_ops::parse_num(rhs),
        "-ne" => crate::test_ops::parse_num(lhs) != crate::test_ops::parse_num(rhs),
        "-lt" => crate::test_ops::parse_num(lhs) < crate::test_ops::parse_num(rhs),
        "-le" => crate::test_ops::parse_num(lhs) <= crate::test_ops::parse_num(rhs),
        "-gt" => crate::test_ops::parse_num(lhs) > crate::test_ops::parse_num(rhs),
        "-ge" => crate::test_ops::parse_num(lhs) >= crate::test_ops::parse_num(rhs),
        "<" => lhs < rhs,
        ">" => lhs > rhs,
        "-nt" | "-ot" | "-ef" => {
            let info_lhs = platform.file_info(lhs);
            let info_rhs = platform.file_info(rhs);
            crate::test_ops::eval_binary_file_test(&info_lhs, &info_rhs, op).unwrap_or(false)
        }
        _ => false,
    })
}
...
fn r#break(exec: &mut Executor, args: &[String]) -> Result<ProcStatus, String> {
    if exec.loop_depth == 0 {
        return Err("muffin: break: only meaningful in a `for`, `while`, or `until` loop".into());
    }
...
fn continue_(exec: &mut Executor, args: &[String]) -> Result<ProcStatus, String> {
    if exec.loop_depth == 0 {
        return Err("muffin: continue: only meaningful in a `for`, `while`, or `until` loop".into());
    }
    let depth = args.get(1).and_then(|s| s.parse().ok()).unwrap_or(1).max(1);
    exec.loop_control = Some(crate::executor::LoopControl {
        is_break: false,
...
fn r#return(exec: &mut Executor, args: &[String]) -> Result<ProcStatus, String> {
    if exec.fn_depth == 0 {
        return Err("muffin: return: can only `return' from a function or sourced script".into());
    }
    let code = match args.get(1) {
        Some(s) => match s.parse::<i32>() {
            Ok(n) => n,
            Err(_) => {
                return Err(alloc::format!(
                    "muffin: return: `{s}': numeric argument required"
                ));
            }
        },
        None => exec.last_status.status_code(),
...
fn source(exec: &mut Executor, args: &[String]) -> Result<ProcStatus, String> {
    let Some(path) = args.get(1) else {
        return Err("muffin: source: filename argument required".into());
    };
    let content = read_file(exec.platform, path).map_err(|e| alloc::format!("muffin: source: {path}: {e}"))?;
...
pub(crate) fn read_file(platform: &dyn muffin_platform::ProcessModel, path: &str) -> Result<String, String> {
    let fd = platform
        .open_file(path, muffin_platform::FileOpenMode::Read)
        .map_err(|e| alloc::format!("{e}"))?;
    let mut out = Vec::new();
    let mut buf = [0u8; 4096];
    loop {
        match platform.read(fd, &mut buf) {
            Ok(0) => break,
...
fn read(exec: &mut Executor, args: &[String]) -> Result<ProcStatus, String> {
    ...
    let vars = &args[i..];
    if vars.is_empty() {
        return Err("muffin: read: usage: read [-r] var...".into());
    }
    let line = read_line(exec.platform, raw)?;
    ...
    for (idx, var) in vars.iter().enumerate() {
        ...
        let _ = exec.env.set(var, muffin_env::EnvVar::new(value));
    }
    Ok(ProcStatus::Exit(0))
}

/// Read one line (up to `\n`, excluded) from fd 0.
pub(crate) fn read_line(platform: &dyn muffin_platform::ProcessModel, raw: bool) -> Result<String, String> {
    let mut out = String::new();
    let mut buf = [0u8; 1];
    let mut escaped = false;
    loop {
        match platform.read(0, &mut buf) {
            Ok(0) => break,
            Ok(_) => {
                let b = buf[0];
                if escaped {
...
fn r#type(exec: &Executor, args: &[String]) -> Result<ProcStatus, String> {
    if args.len() < 2 {
        return Err("muffin: type: usage: type NAME...".into());
    }
    let mut all_ok = true;
    for name in &args[1..] {
        let kind = crate::resolve::describe(exec, name);
        out(exec.platform, &alloc::format!("{name} is {kind}\n"));
        if kind.starts_with("muffin:") {
            all_ok = false;
        }
    }
        ...
        match arg.find('=') {
            Some(eq) => {
                ...
                if !is_identifier(name) {
                    return Err(alloc::format!(
                        "muffin: export: `{arg}': not a valid identifier"
                    ));
                }
                exec.env
                    .set(name, EnvVar::new(val).set_flags(EnvVarFlags::EXPORT))
                    .map_err(|e| alloc::format!("muffin: export: {e}"))?;
            }
            None => {
        ...
        match arg.find('=') {
            Some(eq) => {
                ...
                exec.env
                    ...
                    .map_err(|e| alloc::format!("muffin: readonly: {e}"))?;
            }
            None => {
...
fn shift(exec: &mut Executor, args: &[String]) -> Result<ProcStatus, String> {
    ...
    if n > exec.positional.len().saturating_sub(1) {
        return Err("muffin: shift: shift count out of range".into());
    }
...
fn dirs(exec: &Executor) -> Result<ProcStatus, String> {
    let mut buf = String::new();
    buf.push_str(&exec.platform.current_dir());
    ...
    for a in &args[1..] {
        match a.as_str() {
            ...
            other if other.starts_with('-') => {
                return Err(alloc::format!("muffin: jobs: invalid option `{other}`"));
            }
            _ => return Err(alloc::format!("muffin: jobs: no job name `{a}`")),
        }
    }
...
fn wait(exec: &mut Executor, args: &[String]) -> Result<ProcStatus, String> {
    if args.len() <= 1 {
        ...
        let handles: Vec<muffin_platform::ProcessHandle> = exec
            .background
            .iter()
    ...
    for pid_arg in &args[1..] {
        let Ok(pid) = pid_arg.parse::<i32>() else {
            return Err(alloc::format!("muffin: wait: `{pid_arg}`: not a pid"));
        };
        match exec.reap_job(pid) {
            Some(st) => last = st,
            None => {
                return Err(alloc::format!(
                    "muffin: wait: pid {pid} is not a child of this shell"
                ));
            }
        }
    }
...
///
/// muffin has no terminal job control yet, so like non-interactive bash these
/// report "no job control" (exit 1).
fn fg_bg(exec: &mut Executor, args: &[String], _is_fg: bool) -> Result<ProcStatus, String> {
    if exec.interactive {
        return Err(alloc::format!(
            "muffin: {}: job control not implemented yet",
            args[0]
        ));
    }
    Err(alloc::format!("muffin: {}: no job control", args[0]))
}
    ...
    while i < args.len() {
        ...
        if body == "o" || body == "O" {
            ...
            match name.as_str() {
                ...
                other => {
                    return Err(alloc::format!(
                        "muffin: set: -o: invalid option name `{other}`"
                    ));
                }
            }
            i += 2;
        ...
        for c in body.chars() {
            match c {
                ...
                other => return Err(alloc::format!("muffin: set: invalid option `-{other}`")),
            }
        }
        i += 1;
    }
...
fn shopt(exec: &mut Executor, args: &[String]) -> Result<ProcStatus, String> {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "-O" | "-E" | "-S" => {}
            other if other.starts_with('-') => {
                return Err(alloc::format!("muffin: shopt: invalid option `{other}`"));
            }
            _ => break,
        }
        i += 1;
    ...
    if set || unset {
        for name in &names {
            let Some(slot) = shopt_slot_mut(exec, name) else {
                return Err(alloc::format!("muffin: shopt: no such option `{name}`"));
            };
            *slot = set;
        }
        return Ok(ProcStatus::Exit(0));
...
fn trap(exec: &mut Executor, args: &[String]) -> Result<ProcStatus, String> {
    ...
    i += 1;
    if i >= args.len() {
        return Err("muffin: trap: usage: trap [-lp] [[action] signal ...]".into());
    }
    let mut any = false;
    for name in &args[i..] {
        let Some(trigger) = parse_trigger(name) else {
            return Err(alloc::format!("muffin: trap: invalid signal spec `{name}`"));
        };
        ...
        if action != "-" {
            ...
            if let TrapTrigger::Signal(sig) = trigger {
                ...
                let _ = exec.platform.install_trap_handler(sig);
            }
        }
        any = true;
    }
...
fn parse_trigger(name: &str) -> Option<TrapTrigger> {
    ...
    let sig = match name {
        ...
        "ABRT" => Signal::Abort,
        "BUS" => Signal::Bus,
        "FPE" => Signal::FloatingPoint,
        "SEGV" => Signal::Segmentation,
        "TTIN" => Signal::Ttin,
        "TTOU" => Signal::Ttou,
        _ => {
            return name
...
fn trigger_name(trigger: TrapTrigger) -> alloc::string::String {
    ...
    match trigger {
        ...
        TrapTrigger::Signal(s) => match s {
            Signal::Hangup => "HUP".into(),
            Signal::Interrupt => "INT".into(),
            Signal::Quit => "QUIT".into(),
            Signal::Illegal => "ILL".into(),
            Signal::Abort => "ABRT".into(),
            Signal::Bus => "BUS".into(),
            Signal::FloatingPoint => "FPE".into(),
            Signal::Kill => "KILL".into(),
            Signal::Segmentation => "SEGV".into(),
            Signal::Pipe => "PIPE".into(),
            Signal::Alarm => "ALRM".into(),
            Signal::Terminate => "TERM".into(),
            Signal::Child => "CHLD".into(),
            Signal::Continue => "CONT".into(),
            Signal::Stop => "STOP".into(),
            Signal::Tstp => "TSTP".into(),
            Signal::Ttin => "TTIN".into(),
            Signal::Ttou => "TTOU".into(),
...
mod tests {
    ...
    use muffin_platform::Platform;

    fn mock_platform() -> &'static muffin_platform_mock::MockPlatform {
        Box::leak(Box::new(muffin_platform_mock::MockPlatform::new()))
    }

    #[test]
    fn alias_defines() {
        let mut exec = Executor::new(EnvStack::new(), mock_platform());
    ...
    fn test_binary_and_unary() {
        ...
        // 1 = 2 → false (exit 1)
        assert_eq!(
            test_builtin(p, &a(&["test", "1", "=", "2"])).unwrap(),
            ProcStatus::Exit(1)
        );
        // 1 = 1 → true
        assert_eq!(
            test_builtin(p, &a(&["test", "1", "=", "1"])).unwrap(),
            ProcStatus::Exit(0)
        );
        // 1 != 2 → true
        assert_eq!(
        ...
        // 2 -gt 1 → true
        assert_eq!(
            test_builtin(p, &a(&["test", "2", "-gt", "1"])).unwrap(),
            ProcStatus::Exit(0)
        );
        // -n x → true
        assert_eq!(
            test_builtin(p, &a(&["test", "-n", "x"])).unwrap(),
            ProcStatus::Exit(0)
        );
        // -n "" → false
        assert_eq!(
            test_builtin(p, &a(&["test", "-n", ""])).unwrap(),
            ProcStatus::Exit(1)
        );
        // ! expression
        assert_eq!(
        ...
        // [ 1 = 1 ] → true
        assert_eq!(
            test_builtin(p, &a(&["[", "1", "=", "1", "]"])).unwrap(),
            ProcStatus::Exit(0)
        );
        // bare `test` with no args → false
        assert_eq!(test_builtin(p, &a(&["test"])).unwrap(), ProcStatus::Exit(1));
    }

    #[test]
    fn test_file_type_operators() {
        let p = mock_platform();
        let file_info = |exists: bool,
                         is_file: bool,
                         is_dir: bool,
                         is_symlink: bool,
                         is_socket: bool,
        ...
        p.set_file_info("/f", file_info(true, true, false, false, false, false));
        p.set_file_info("/d", file_info(true, false, true, false, false, false));
        p.set_file_info("/link", file_info(true, false, false, true, false, false));
        p.set_file_info("/sock", file_info(true, false, false, false, true, false));
        p.set_file_info("/fifo", file_info(true, false, false, false, false, true));
        let a = |v: &[&str]| v.iter().map(|s| s.to_string()).collect::<Vec<_>>();
        let t = |args: &[&str]| test_builtin(p, &a(args)).unwrap();
        // -e: existence
        assert_eq!(t(&["test", "-e", "/f"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-e", "/missing"]), ProcStatus::Exit(1));
        // -f / -d discriminate
        assert_eq!(t(&["test", "-f", "/f"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-f", "/d"]), ProcStatus::Exit(1));
        assert_eq!(t(&["test", "-d", "/d"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-d", "/f"]), ProcStatus::Exit(1));
        // symlink / socket / fifo
        assert_eq!(t(&["test", "-L", "/link"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-h", "/link"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-L", "/f"]), ProcStatus::Exit(1));
        assert_eq!(t(&["test", "-S", "/sock"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-S", "/f"]), ProcStatus::Exit(1));
        assert_eq!(t(&["test", "-p", "/fifo"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-p", "/f"]), ProcStatus::Exit(1));
    }

    #[test]
    fn test_file_permission_operators() {
        let p = mock_platform();
        p.set_file_info(
            "/f",
            muffin_platform::FileInfo {
        ...
        let a = |v: &[&str]| v.iter().map(|s| s.to_string()).collect::<Vec<_>>();
        let t = |args: &[&str]| test_builtin(p, &a(args)).unwrap();
        assert_eq!(t(&["test", "-r", "/f"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-w", "/f"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-x", "/f"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-r", "/other"]), ProcStatus::Exit(1));
        assert_eq!(t(&["test", "-s", "/f"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-s", "/other"]), ProcStatus::Exit(1));
        assert_eq!(t(&["test", "-u", "/f"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-g", "/f"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-k", "/f"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-u", "/other"]), ProcStatus::Exit(1));
        // -N: modified since last read (mtime > atime)
        assert_eq!(t(&["test", "-N", "/f"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-N", "/other"]), ProcStatus::Exit(1));
        // -O / -G: owned by euid / egid (mock runs as 0:0)
        assert_eq!(t(&["test", "-O", "/f"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-O", "/other"]), ProcStatus::Exit(1));
        assert_eq!(t(&["test", "-G", "/f"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "-G", "/other"]), ProcStatus::Exit(1));
        // -t: mock has no terminal
        assert_eq!(t(&["test", "-t", "0"]), ProcStatus::Exit(1));
    }

    #[test]
    fn test_file_binary_operators() {
        let p = mock_platform();
        let stamp = |mtime: i64, dev: u64, ino: u64| muffin_platform::FileInfo {
            exists: true,
            is_file: true,
            mtime,
            atime: 0,
        ...
        p.set_file_info("/new", stamp(300, 1, 10));
        p.set_file_info("/old", stamp(100, 1, 20));
        // Same dev+ino as /new (hardlink).
        p.set_file_info("/hardlink", stamp(300, 1, 10));
        let a = |v: &[&str]| v.iter().map(|s| s.to_string()).collect::<Vec<_>>();
        let t = |args: &[&str]| test_builtin(p, &a(args)).unwrap();
        assert_eq!(t(&["test", "/new", "-nt", "/old"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "/old", "-nt", "/new"]), ProcStatus::Exit(1));
        assert_eq!(t(&["test", "/old", "-ot", "/new"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "/new", "-ot", "/old"]), ProcStatus::Exit(1));
        assert_eq!(t(&["test", "/new", "-ef", "/hardlink"]), ProcStatus::Exit(0));
        assert_eq!(t(&["test", "/new", "-ef", "/old"]), ProcStatus::Exit(1));
    }
    ...
    fn cd_updates_pwd_and_oldpwd() {
        let p = mock_platform();
        p.set_cwd("/start");
        let mut exec = Executor::new(EnvStack::new(), p);
        let r = cd(&mut exec, &["cd".into(), "/work".into()]).unwrap();
        assert_eq!(r, ProcStatus::Exit(0));
        assert_eq!(p.current_dir(), "/work");
        assert_eq!(exec.env.get("PWD").map(|v| v.value()), Some("/work"));
        assert_eq!(exec.env.get("OLDPWD").map(|v| v.value()), Some("/start"));
    }

    #[test]
    fn cd_dash_returns_to_oldpwd_and_prints_it() {
        let p = mock_platform();
        p.set_cwd("/start");
        let mut exec = Executor::new(EnvStack::new(), p);
        cd(&mut exec, &["cd".into(), "/work".into()]).unwrap();
        let r = cd(&mut exec, &["cd".into(), "-".into()]).unwrap();
        assert_eq!(r, ProcStatus::Exit(0));
        assert_eq!(p.current_dir(), "/start");
        // `cd -` prints the directory it switches to.
        assert_eq!(p.written_to(1), b"/start\n");
    }
    ...
    fn cd_without_home_errors() {
        let p = mock_platform();
        let mut exec = Executor::new(EnvStack::new(), p);
        let r = cd(&mut exec, &["cd".into()]);
        assert!(r.is_err());
    }

    #[test]
    fn pwd_writes_current_dir() {
        let p = mock_platform();
        p.set_cwd("/work");
        let r = pwd(p).unwrap();
        assert_eq!(r, ProcStatus::Exit(0));
        assert_eq!(p.written_to(1), b"/work\n");
    }

    #[test]
    fn signal_trigger_round_trip() {
        // Every name parse_trigger accepts maps back to itself.
        let names = [
            "HUP", "INT", "QUIT", "ILL", "ABRT", "BUS", "FPE", "KILL", "SEGV", "PIPE",
            "ALRM", "TERM", "CHLD", "CONT", "STOP", "TSTP", "TTIN", "TTOU", "WINCH",
            "USR1", "USR2",
        ];
        for name in names {
            let trigger = parse_trigger(name).expect("known signal name");
            assert_eq!(trigger_name(trigger), name);
        }
        // Non-signal triggers keep their names.
        assert_eq!(trigger_name(TrapTrigger::Exit), "EXIT");
        assert_eq!(trigger_name(TrapTrigger::Err), "ERR");
        assert_eq!(trigger_name(TrapTrigger::Debug), "DEBUG");
        // Numeric input survives as Other(n).
        let trigger = parse_trigger("9").expect("numeric signal");
        assert_eq!(trigger_name(trigger), "9");
        // Unknown names are rejected.
        assert!(parse_trigger("NOTASIG").is_none());
    }
}
