...
//!
//! Tokenizes a source line with [`muffin_syntax::lexer`] and inserts ANSI
...
use muffin_syntax::Token;
use muffin_syntax::lexer::{LexContext, Lexer};
use muffin_syntax::token::TokenKind::*;
...
fn update_ctx(lexer: &mut Lexer, tok: &Token) {
    use muffin_syntax::lexer::LexContext;
    match tok.kind {
        Word => {
            if matches!(
                tok.text.as_str(),
