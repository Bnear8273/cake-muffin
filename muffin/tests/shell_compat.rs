//! Differential tests: run a snippet through both `muffin` and real `bash` and
...
/// Path to the built `muffin` binary (provided by cargo for integration tests).
...
/// Assert `muffin` and `bash` agree on stdout and exit code for `src`.
