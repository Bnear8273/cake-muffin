...
//!
//! `muffin` with no arguments enters this loop: a PS1 prompt, line editing and
...
use muffin_complete::CompleteKind;
use muffin_editor::Candidate;
use muffin_editor::Editor as LineEditor;
use muffin_editor::{Key, KeyParser};
use muffin_exec::Executor;
use muffin_platform::XdgKind;
...
fn complete_in(exec: &Executor, line: &str, pos: usize) -> (usize, Vec<Candidate>) {
    let (word_start, word) = muffin_complete::current_word(line, pos);
    let kind = muffin_complete::classify(line, pos);
    let platform = exec.platform;

    match kind {
        CompleteKind::Command => {
            let mut cands: Vec<String> = Vec::new();
            for b in muffin_exec::builtins::builtin_names() {
            ...
            let cands: Vec<String> = cands
                ...
                .collect();
            let matches = dedup(muffin_complete::filter_candidates(word, &cands));
            let pairs = matches
                .into_iter()
                .map(|c| Candidate {
                    display: c.clone(),
        ...
        CompleteKind::File => {
            ...
            let matches = dedup(muffin_complete::filter_candidates(base, &cands));
            let pairs = matches
                .into_iter()
                .map(|c| Candidate {
                    display: c.clone(),
        ...
        CompleteKind::Variable => {
            let dollar = muffin_complete::dollar_in_word(word).unwrap_or(0);
            let prefix = word[dollar + 1..].trim_end_matches(['"', '\'']);
            let names: Vec<String> = exec.env.get_names().iter().map(|n| n.to_string()).collect();
            let matches = dedup(muffin_complete::filter_candidates(prefix, &names));
            let pairs = matches
                .into_iter()
                .map(|c| Candidate {
                    display: format!("${c}"),
...
fn edit_line(
    ...
    let highlight = |s: &str| {
        ...
        let found = |name: &str| {
            exec.aliases.contains_key(name)
                || !matches!(
                    muffin_exec::resolve::resolve_command(&exec, name),
                    muffin_exec::resolve::CommandSpec::NotFound
                )
        };
        muffin_highlight::highlight_line(s, &found)
    };
    let hint = |line: &str, hist: &[String]| {
        ...
        let filtered: Vec<String> = hist
            ...
            .cloned()
            .collect();
        muffin_reader::suggest(line, &filtered)
    };
...
pub fn run_interactive() -> ! {
    let executor = Rc::new(RefCell::new(Executor::new(import_env(), muffin_platform::get())));
    {
        ...
        // scripts only when `shopt -s expand_aliases` is set, which muffin
        // doesn't do).
    ...
    // When stdin is not a terminal (e.g. `echo 'ls' | muffin`), read plain
    ...
    loop {
        ...
        loop {
            ...
            match muffin_syntax::parse(&buffer) {
                Ok(_) => {
                    ...
                    let start = muffin_platform::get().time_nanos();
                    let outcome = {
                        let mut exec = executor.borrow_mut();
                        exec.eval_str(&buffer)
                    };
                    last_elapsed_ms =
                        Some(muffin_platform::get().time_nanos().saturating_sub(start) / 1_000_000);
                ...
                Err(errs) => {
                    ...
                    eprintln!("muffin: {}", errs[0].message);
                    break;
                }
            }
        }
...
fn data_dir() -> PathBuf {
    PathBuf::from(muffin_platform::get().xdg_dir(XdgKind::Data)).join("muffin")
}
...
/// Evaluate the startup rc file (`$XDG_DATA_HOME/muffin/rc`) if it exists.
