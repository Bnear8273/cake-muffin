...
//! read-paint loop that drives `muffin_editor::Editor`. All byte I/O goes
//! through the global `Platform` so the logic stays portable; only this thin
//! driver touches terminal state.

use muffin_editor::{Candidate, EditEvent, Editor, Key, KeyParser, Render, Services};
use muffin_platform::{PlatformError, ProcessModel, TerminalState};
...
pub(crate) fn read_loop<'a>(
    ...
    let platform = muffin_platform::get();
    let _guard = RawModeGuard::enter().map_err(|e| format!("muffin: {e}"))?;
    let cols = platform
        .terminal_size(0)
        ...
        while let Some(k) = pending.first().copied() {
            ...
            match editor.handle(k, &srv) {
                ...
                EditEvent::Submit(line) => {
                    ...
                    let platform = muffin_platform::get();
                    let _ = platform.write(1, &out).map_err(|e| format!("muffin: {e}"))?;
...
fn transient_collapse(out: &mut Vec<u8>, line: &str, prev_rows: usize, cols: usize) {
    if prev_rows == 2 {
        ...
        let t = muffin_editor::render::truncate_ansi_to_cols(line, cols.saturating_sub(2));
