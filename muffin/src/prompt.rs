...
//! calls [`muffin_prompt::render`] to produce the two-line ANSI prompt. The repl
//! driver decides whether to use this or the legacy PS1 prompt.

use muffin_exec::Executor;
use muffin_prompt::{Config, Facts, GitStatus};

/// Whether the segmented prompt is enabled. On by default; an explicitly
/// empty `MUFFIN_PROMPT_LEFT` disables it (falling back to the legacy
/// PS1/path prompt).
pub fn prompt_enabled(exec: &Executor) -> bool {
    match exec.env.get("MUFFIN_PROMPT_LEFT") {
...
pub fn build_prompt(exec: &Executor, elapsed_ms: Option<u64>) -> (String, String) {
    ...
    let home_str = exec
        ...
        .filter(|s| !s.is_empty())
        .map(str::to_owned);
    let dir_str = shorten_dir(&muffin_platform::get().current_dir(), home_str.as_deref());
    let git = capture_git_status();
    let exit = exec.last_status.status_code();
    let signal = match &exec.last_status {
        muffin_exec::ProcStatus::Signal(n) => Some(muffin_platform::get().signal_name(*n)),
        _ => None,
    };
    let clock = {
        let (h, m, s) = muffin_platform::get().local_time_hms();
    ...
    let r = muffin_prompt::render(&facts, &cfg);
...
fn capture_git_status() -> Option<GitStatus> {
    ...
    muffin_prompt::parse_status(text)
}
