//! The muffin driver binary.
//!
//! This is the only crate that links the OS layer (`muffin-platform-unix`) into
...
use muffin_env::{EnvStack, EnvVar, EnvVarFlags};
use muffin_exec::Executor;
use muffin_platform_unix::unix_backend;
...
fn main() {
    ...
    muffin_platform::init(unix_backend());
    ...
    match parse_args(&args) {
        ...
        ParsedArgs::Error(msg) => {
            eprintln!("muffin: {msg}");
            eprintln!("Try 'muffin --help' for more information.");
            std::process::exit(2);
        }
        ParsedArgs::Command(cmd, rest) => {
            ...
            if rest.is_empty() {
                executor.positional = vec!["muffin".into()];
            } else {
...
/// Argument parsing: `muffin -c "command" args...`, `muffin --help`, or plain
/// `muffin` for interactive mode.
...
fn print_help() {
    println!("muffin - a bash-compatible shell with a modern interactive layer");
    println!();
    println!("Usage: muffin [OPTIONS] [ARG...]");
    println!("       muffin -c 'COMMAND' [ARG...]");
    println!();
    println!("With no arguments, muffin starts an interactive shell.");
    println!();
    println!("Options:");
    ...
    println!();
    println!("Builtins:");
    let names = muffin_exec::builtins::builtin_names();
